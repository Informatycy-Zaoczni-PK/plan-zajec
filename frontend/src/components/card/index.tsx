...
const ClassCard: React.FC<IClassCard> = ({ lesson, groups }) => {
  ...
  if (name.includes("ćwiczenia") || name.includes("ćw")) {
    classType = "c";
    cardColor = "3";
    ...
    <div>
      {isInClassGroup && (
        <Card cardColor={cardColor}>
          ...
          <p className="size-l color-light_1">{name}</p>
        </Card>
      )}
    </div>
  );
...
const Card: React.FC<ICard> = ({ cardColor, children }) => {
  return <div className={`card card-bg_${cardColor}`}>{children}</div>;
};

export { ClassCard, Card };
